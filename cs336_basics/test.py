...
class BPETokenizer:
    def __init__(self):
        self.config = Config(
            path = rf"E:\courses\cs336\assignment1\data\TinyStoriesV2-GPT4-valid.txt",
    ...
    @staticmethod
    def subprocess(path: str, chunk_boundary: List, PAT: str, read_unit_size: int):
        import regex as re
        from collections import defaultdict
        ...
        with open(path, 'rb') as f:
            ...
            for match in re.finditer(PAT, chunk.decode("utf-8", errors="ignore")):
    ...
    def pre_tokenize(self):
        ...
        with ProcessPoolExecutor(self.config.pre_tokenize.MAX_WORKERS) as executor:
            futures = [
                executor.submit(
                    BPETokenizer.subprocess,
                    self.config.path,
                    chunk_boundary,
                    self.PAT,
                    self.config.pre_tokenize.READ_UNIT_SIZE
                )
                for chunk_boundary in chunk_boundaries
            ]
...
if __name__ == '__main__':
    bpe = BPETokenizer()
    result = bpe.pre_tokenize()
    total = {}
    for r in result:
        total.update(r)
    m = sorted(total.keys(), key=lambda x: total[x])
    print(m[-10:])